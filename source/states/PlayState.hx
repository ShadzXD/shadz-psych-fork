...
class PlayState extends MusicBeatState
{
	...
	public dynamic function updateScore(miss:Bool = false, scoreBop:Bool = true)
	{
		...
		if (scoreBop && ClientPrefs.data.scoreZoom)
			hudClass.doScoreBop();
	...
	function startSong():Void
	{
		...
		if (autoUpdateRPC)
			DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", hudClass?.iconP2?.getCharacter(), true, songLength);
		#end
	...
	function resetRPC(?showTime:Bool = false)
	{
		...
		if (showTime)
			DiscordClient.changePresence(detailsText, SONG.song
				+ " ("
				+ storyDifficultyText
				+ ")", hudClass?.iconP2?.getCharacter(), true,
				songLength
	...
	function doDeathCheck(?skipHealthCheck:Bool = false)
	{
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead && gameOverTimer == null)
		{
			...
			if (ret != LuaUtils.Function_Stop)
			{
				...
				if (autoUpdateRPC)
					DiscordClient.changePresence("Game Over - " + detailsText, SONG.song + " (" + storyDifficultyText + ")", hudClass?.iconP2?.getCharacter());
				#end
				isDead = true;
				return true;
			}
